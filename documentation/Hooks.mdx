...
<p>
  ...
  , the one which you'll probably use most is the declarative and render-prop
  driven way which is front and center. But there is a second, imperative way
  which you can either use raw or with the help of Reacts new "hooks" feature
  which establishes a class-less component architecture (currently in alpha, and
  you'll need both react@next and react-dom@next to use it).
</p>
...
function ClasslessComponent() {
  ...
  return (
    <animated.div
      style={{
        transform: props.pos.interpolate(
          (x, y) => `translate3d(${x}px,${x}px,0)`
        ),
      }}>
...
You can also create natural trails by tying your hooks together. Whenever you update the first spring, either by overwriting it or by calling `set`, the others will naturally follow:
