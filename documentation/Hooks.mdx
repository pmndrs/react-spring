...
<p>
  <h3><span className="highlight"><i>Either</i>: overwrite values</span> to change the animation</h3>
</p>
...
<p>
  <h3><span className="highlight"><i>Or</i>: pass a function</span> that returns values, and update using "set"</h3>
</p>
...
<p>
  <h3><span className="highlight"><i>Finally</i>: distribute animated props</span> among the view</h3>
</p>
...
## useKeyframes
 ...
 `useKeyframes.[spring | trail](Config | Function | [Configs]): useFunction(SlotName)`
...
```jsx
const useSlot = useKeyframes.trail(values)
