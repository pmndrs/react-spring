...
Therefore, to reduce the impact of changes, `react-spring` offers a completely different entry point to work with Hooks, so to not interfere with the stable releases.
