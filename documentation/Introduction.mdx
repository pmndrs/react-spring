...
import { RewindSpringProvider, RewindSpring } from './'
...
<RewindSpringProvider>
  <p>
    A <code>Spring</code> moves data from one state to another. This will be your primary tool.
  </p>
...
<Spring from={{ opacity: 0 }} to={{ opacity: 1 }}>
  ...
  <p>
    A <code>Transition</code> animates component lifecycles. It takes a <span className="highlight">list of items</span> and their <span className="highlight">keys</span>. Whenever the list changes, items are added, removed, reordered or updated, it will help you to animate these changes.
  </p>
...
const items = [
  { key: 0, text: 'h'}, { key: 1, text: 'e'},
  { key: 2, text: 'l'}, { key: 3, text: 'l'},
  { key: 4, text: 'o'},
]
...
<Transition
  ...
  {toggle =>
    toggle
      ? props => <div style={props}>yes</div>
      : props => <div style={props}>no</div>
  }
</Transition>`}/>
    <RewindSpring>
      {x => <animated.div style={{ opacity: x }}>yes</animated.div>}
    </RewindSpring>
  </div>
  ...
  leave={{ opacity: 0 }}>
  {show =>
    show && (props => <div style={props}>hello</div>)
  }
</Transition>`}/>
    <RewindSpring>
      {x => <animated.div style={{ opacity: x }}>hello</animated.div>}
  ...
  <p>
    A <code>Trail</code> animates the first child of a list of elements, the rest follow the spring of their previous sibling.
  </p>
...
- Routing (react-router): https://codesandbox.io/embed/mjnwrk1o3p
...
- Card flip: https://codesandbox.io/embed/jn3z28ry33
