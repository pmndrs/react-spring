...
<RewindSpringProvider>
  ...
  <p>
    <span className="highlight">Do not think of the values you pass as "styles"</span> per se, although you can use them as such.
  </p>
...
<DemoGrid padding={0}>
  {examples.filter(data => data.tags.includes('springs')).map(data => (
    <Demo
      key={data.name}
      {...data}
