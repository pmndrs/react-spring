...
You can use this hook to define and update data. The data you pass in consists of the same props you would normally give a spring, with the distinction that unknown values are interpolated to `to` by default, similar to how [Keyframes](/keyframes) roll. You can drop in everything, just like the a Springs `to` props it understands numbers, percentages, strings, colors and so on. See [/spring#interpolation](/spring#interpolation).
