...
component you wrote — for example, the `springs` of a `usePresenceList`
entry — the receiving prop has to be typed as `SpringValues`, not
`React.CSSProperties`. Each entry is a `SpringValue` instance, not the
underlying primitive.

```tsx
import { animated, SpringValues, usePresenceList } from '@react-spring/web'
...
const List = ({ items }: { items: number[] }) => {
  ...
  return entries.map(({ key, springs }) => <Item key={key} styles={springs} />)
}
```
