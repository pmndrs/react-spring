...
[`usePresenceList`](/docs/components/use-presence-list) and
[`usePresence`](/docs/components/use-presence) don't accept a top-level
`onResolve`. Set it inside `enter`, `update` or `leave` instead.
...
Called after a [`useTransition`](/docs/components/use-transition) item is unmounted. Not supported by `usePresenceList` or `usePresence`.
...
The `Item` argument is only present when using the relevant events within the [`usePresenceList`](/docs/components/use-presence-list) or [`useTransition`](/docs/components/use-transition) hooks.
