...
animation hook e.g. [`useSpring`](/docs/components/use-spring) & [`usePresenceList`](/docs/components/use-presence-list)
...
This will first run the `useSpring` hook and then the `usePresenceList` hook when the component has mounted and the `useSpring` has come to rest.
...
function MyComponent() {
  ...
  return (
    ...
    >
      {entries.map(({ key, item, springs: style }) => (
        <animated.div
          key={key}
          style={{
            width: '120px',
