---
meta:
  title: R3F | React Spring
  'og:title': R3F | React Spring
  'twitter:title': R3F | React Spring
  description: A detailed guide to using React Spring with R3F.
  'og:description': A detailed guide to using React Spring with R3F.
  'twitter:description': A detailed guide to using React Spring with R3F.
  'og:url': https://beta.react-spring.dev/docs/guides/react-three-fiber
  'twitter:url': https://beta.react-spring.dev/docs/guides/react-three-fiber
sidebar_position: 1
---
...
either, check out our [getting started](/docs/getting-started) or alternatively, the [r3f](https://docs.pmnd.rs/react-three-fiber) docs.
:::

## Introduction

In this guide we'll explore why `react-spring` is a valuable addition to your `r3f` project, working with the [`imperative API`](/docs/concepts/imperative-api)
...
to access the material object and use the `THREE.Color.lerp` function, slowly incrementing by lerp value until the color is reached. But this then
...
Why not read more about it the [imperative](/docs/concepts/imperative-api) and [interpolation](/docs/advanced/interpolation) APIs?
...
Instead, you can use the `onChange` event handler to `get` the value of your springs and react to them accordingly. The code below is a conveluted example
