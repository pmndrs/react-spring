...
We're going to explore the latter, we'll start by changing the notation of our hook:
...
argument as our `springs` which we're already used to and the second argument is the
`api` that controls these springs.
